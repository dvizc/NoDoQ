...
function hash(message) {
    return (BigInt(crypto.keccak256(utils.toBeHex(message, 32))) >> 8n).toString();
}
...
async function generateProof(identity, groupOrMerkleProof, message, scope, merkleTreeDepth, snarkArtifacts) {
    ...
    snarkArtifacts ?? (snarkArtifacts = await artifacts.maybeGetSnarkArtifacts(artifacts.Project.SEMAPHORE, {
